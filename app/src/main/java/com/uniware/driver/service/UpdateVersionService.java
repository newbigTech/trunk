package com.uniware.driver.service;
...
public class UpdateVersionService {
	 ...
	 *            上下文
	 */
	public UpdateVersionService( Context context,String s,boolean must) {
		super();
	...
	public void checkUpdate1(String code) {
		if (isUpdate(code)) {
			if(must)
			{
				mustShowUpdateVersionDialog();
	...
	private void mustShowUpdateVersionDialog() {
		// 构造对话框
		MyDialog.Builder builder = new MyDialog.Builder(context);
		builder.setTitle("软件更新");
		builder.setMessage(myMsg);
...
//			});
			...
			downLoadDialog.setView(v, 0, 0, 0, 0);
			downLoadDialog.show();
			// 现在文件
			downloadApk();
		}
	 ...
	 * @return
	 */
	private boolean isUpdate(String wel) {
		...
		hashMap.put("versionCode", wel);
		hashMap.put("fileName", "移步叫车司机");
	  hashMap.put("loadUrl", "http://223.72.209.142:9000/905platform/export/last.apk");
		if (null != hashMap) {
	...
	private int getVersionCode(Context context2) {
		...
		try {
			...
			versionCode = context.getPackageManager().getPackageInfo("com.uniware.driver", 0).versionCode;
	...
	public void uninstallAPK() {
		Uri packageURI = Uri.parse("package:com.yourong.caiyan");
