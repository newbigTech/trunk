...
public abstract class UseCase {
  ...
  public void schedule(long initialDelay, Subscriber UseCaseSubscriber) {
    //noinspection unchecked
    this.subscription = Observable.timer(initialDelay, TimeUnit.SECONDS, Schedulers.io())
  ...
  public void schedule(long initialDelay, long period, Subscriber UseCaseSubscriber) {
    //noinspection unchecked
    this.subscription = Observable.interval(initialDelay, period, TimeUnit.SECONDS, Schedulers.io())
        .flatMap(new Func1<Long, Observable<?>>() {
          @Override public Observable<?> call(Long aLong) {
            System.out.println("schedule:" + Thread.currentThread().getName());
            return buildUseCaseObservable();
